...
class transactions
{
    ...
    function transaction8($paciente_id){
        ...
        $resultado =  $obtenerHistorial->fetchAll(\PDO::FETCH_ASSOC);
        $conS1 = null;
        return $resultado;
    }
    ...
    function transaction9($datosConsulta){
        ...
        try{
            ...
            $insertarConsulta = $conS3->prepare("INSERT INTO consulta (hora, fecha, notas, estudiante_id, paciente_id) VALUES (?,?,?,?,?);");
    ...
    function transaction16(){
        $conS1 = MyConnection::createConnectionS1();

        $obtenerEnfoques = $conS1->prepare("SELECT * FROM enfoque;");

        $obtenerEnfoques->execute();

        $resultado = $obtenerEnfoques->fetchAll(\PDO::FETCH_ASSOC);

        return $resultado;

    }

    function transaction17(){
        $conS1 = MyConnection::createConnectionS1();

        $obtenerPacientes = $conS1->prepare("SELECT * FROM paciente WHERE activo = 1;");

        $obtenerPacientes->execute();

        $resultado = $obtenerPacientes->fetchAll(\PDO::FETCH_ASSOC);
